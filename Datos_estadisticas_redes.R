...
years <- list("2019" = "2019 in 1000 USD", "2020" = "2020 in 1000 USD", "2021" = "2021 in 1000 USD", "2022" = "2022 in 1000 USD", "2023" = "2023 in 1000 USD")
data_net <- data.frame()
...
write.xlsx(data_net, "data_net_2019_2020_2021_2022_2023.xlsx")
...
names(Paises_atributos_segmentado)
...
for (year in names(years)) {
    ...
    Paises_atributos_segmentado <- Paises_atributos_segmentado[order(Paises_atributos_segmentado$`GDP per capita (current US$) [NY.GDP.PCAP.CD]`, decreasing = TRUE), ]
...
### @Filling Na's ###########################################################
# Reemplace el valor "2022", por el valor del año a imputar (2019,2020,2021,2022,2023)
...
### @Clusters, community detection ######################################################
 ...
 for (graph_name in names(g)){
   ...
   # Fix: Remove vertices with zero strength/degree
   isolated_nodes <- V(graph)[strength(graph) == 0]
   if (length(isolated_nodes) > 0) {
     graph <- delete_vertices(graph, isolated_nodes)
   }
   
   clusters[[graph_name]] <- list()
   clusters[[graph_name]]$c_wt <- cluster_walktrap(graph)
   clusters[[graph_name]]$c_lp <- cluster_label_prop(graph)
   clusters[[graph_name]]$c_im <- cluster_infomap(graph)
   
   # Note on spinglass: It requires a single connected component. 
   # If your graph is disconnected, wrap this in a tryCatch or check component count.
   clusters[[graph_name]]$c_sg <- tryCatch(
     cluster_spinglass(graph), 
     error = function(e) { message("Spinglass failed for ", graph_name, ": ", e$message); return(NULL) }
   )
 }

## @Gráfico por año ###################################################################################################### 
year <- "2022" # Reemplace por el año a graficar (2018,2020,2022)
x11()
plot(g[[year]],
     layout = l[[year]]$l_she,
...
g_filtrado_x_peso <- data_net %>% 
  select(ReporterISO3, PartnerISO3, "2022") %>% # Reemplace por el año a graficar 
  arrange(desc(`2022`)) %>% # Reemplace por el año a graficar 
...
# 1. Expand the layout coordinates to spread nodes out
l_g_filtrado_expanded <- layout_nicely(g_filtrado_x_peso) * 2

x11()
par(mar = c(1, 1, 2, 1)) # Slightly more margin to prevent labels cutting off
plot(x = c_im, 
     y = g_filtrado_x_peso, 
     layout = l_g_filtrado_expanded, # Use expanded layout
     mark.groups = communities(c_im), # Explicitly show cluster shapes if desired
     vertex.size = 18,               # Made nodes bigger
     vertex.frame.color = "white",   # White frame makes colors pop more
     edge.color = adjustcolor("grey20", 0.5), 
     vertex.label.cex = 0.8,         # Slightly smaller text to fit
     vertex.label.color = "black",
     vertex.label.dist = 0,          # 0 puts label inside node; 1-1.5 puts it outside
     edge.arrow.size = 0.4, 
     edge.arrow.width = 0.5, 
     edge.width = 0.8,
     main = "International Trade Network (2022)")


# Increase the area parameter to force nodes further apart for PartnerISO3 only
l_g_filtrado_expanded <- layout_with_fr(g_filtrado_x_peso, 
                                        area = vcount(g_filtrado_x_peso)^4, 
                                        repulserad = vcount(g_filtrado_x_peso)^4)

x11()
par(mar = c(1, 1, 2, 1)) 
plot(x = c_im, 
     y = g_filtrado_x_peso, 
     layout = l_g_filtrado_expanded, 
     rescale = TRUE,                  # Keep this TRUE to auto-fit the window
     mark.groups = communities(c_im), 
     vertex.size = 10,                # Smaller nodes create more "white space"
     vertex.frame.color = "white",
     edge.color = adjustcolor("grey20", 0.3), # Lighter edges reduce visual noise
     vertex.label.cex = 0.6,          # Smaller labels prevent overlapping text
     vertex.label.color = "black",
     vertex.label.dist = 0,           
     edge.arrow.size = 0.2,           # Smaller arrows for a cleaner look
     edge.width = 0.4,
     main = "International Trade Network (2020)")
